...
const api = {
    ...
    export_Dashboard_csv: '/dashboard/export_csv',

    // analytics
    get_Analytics_revenue: '/analytics/revenue',
    get_Analytics_top_products: '/analytics/top_products',
    get_Analytics_low_stock: '/analytics/low_stock',
    get_Analytics_summary: '/analytics/summary',
    get_Analytics_near_expiry: '/analytics/near_expiry',

    // batches (logistics)
    get_Batches: '/delivery/batches/get_batches',
    get_Batch: '/delivery/batches/get_batch/',

    // worker-only product quantity update
    update_Product_quantity: '/inventory/product/update_quantity/',

};

// Prepend base_api or ml_api to each endpoint
const modifiedEndpoints = {};
Object.keys(api).forEach((key) => {
    if (key.startsWith('ml_')) {
        modifiedEndpoints[key] = ml_api + api[key];
    } else {
