...
function UserAccounts() {
    ...
    const handleChange = (e) => {
        ...
        switch(name) {
            ...
            case 'username':
                ...
                break;
            case 'password':
                if (editIndex === 0 && nullCheck(value)) {
                    setFormDataError((prevError) => ({ ...prevError, [name]: true }));
                    setFormDataHelperText((prevText) => ({ ...prevText, [name]: empty_field_warning }));
                } else if (editIndex === 0 && value.length < 8) {
                    setFormDataError((prevError) => ({ ...prevError, [name]: true }));
                    setFormDataHelperText((prevText) => ({ ...prevText, [name]: 'Password must be at least 8 characters.' }));
                } else {
                    setFormDataError((prevError) => ({ ...prevError, [name]: false }));
                    setFormDataHelperText((prevText) => ({ ...prevText, [name]: '' }));
                }
    ...
    const handleSubmit = (e) => {
        ...
        for (const field in formData) {
            if (field !== 'id' && !(editIndex === 1 && field !== 'role')) {
            ...
            if (editIndex === 1) {
                ...
                .then(response => {
                    ...
                    if (response?.data?.generated_password) {
                        toast.info(`Generated password: ${response.data.generated_password}`);
                    }
                })
                .catch(error => {
                    setLoading(false);
                        ...
                        {editIndex === 0 && (
                            <Grid item>
                                <TextField
                                    ...
                                    helperText={formDataHelperText.password || 'Minimum 8 characters'}
                                    type="password"
                                    onChange={handleChange}
                                    fullWidth
                                />
