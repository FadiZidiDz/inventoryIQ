...
function Warehouse() {
    ...
    const handleChange = (event) => {
        ...
        if (name === 'person_conno') {
            const dz_mobile_regex = /^(0[5-7]\d{8}|\+213[5-7]\d{8})$/;
            if (value === '') {
                setErrorHelper(name, true, 'Please fill up required field!', setFormError, setFormHelper);
            } else if (!dz_mobile_regex.test(value)) {
                setErrorHelper(
                    name,
                    true,
                    "Please enter a valid Algerian mobile number. It should start with '05', '06', or '07' or '+2135', '+2136', '+2137' followed by 8 digits.",
                    setFormError,
                    setFormHelper
                );
            } else {
