...
function Register() {
    ...
    const handleChange = (e) => {
        ...
        const strong_regex_pass = /^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#$%^&*]).{8,}$/;
        ...
        if (name === 'password') {
            ...
            } else if (value.length < 8 || !strong_regex_pass.test(value)) {
                setFormDataError((prevError) => ({ ...prevError, password: true, }));
                setFormDataHelperText((prevText) => ({ ...prevText, password: 'Password is too weak! Minimum 8 characters.' }));
            } else {
