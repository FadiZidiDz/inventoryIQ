...
Route::middleware(['auth:api'])->prefix('user')->group(function() {
    ...
    # boss only user management
    Route::middleware('boss')->group(function() {
        Route::get('get_users', [UserController::class, 'get_users_list'])->name('user.getAll');
        Route::get('get_roles', [UserController::class, 'get_roles'])->name('user.getRoles');
        Route::post('create_user', [UserController::class, 'create_user'])->name('user.create');
        Route::get('get_user/{user_id}', [UserController::class, 'get_user'])->name('user.get');
        Route::put('update_user/{user_id}', [UserController::class, 'update_user_role'])->name('user.updateRole');
        Route::delete('remove_user/{user_id}', [UserController::class, 'remove_user'])->name('user.remove');
        Route::delete('disable_user/{user_id}', [UserController::class, 'remove_user'])->name('user.disable');
    });
});
...
Route::middleware(['auth:api', 'boss'])->prefix('audit_trail')->group(function() {
...
Route::middleware(['auth:api'])->prefix('dashboard')->group(function () {
    ...
    Route::get('export_csv', [DashboardController::class, 'export_csv'])->middleware('boss')->name('dashboard.exportCsv');
});

# Routes for Analytics
Route::middleware(['auth:api', 'boss'])->prefix('analytics')->group(function () {
    Route::get('revenue', [AnalyticsController::class, 'revenue'])->name('analytics.revenue');
    Route::get('top_products', [AnalyticsController::class, 'top_products'])->name('analytics.topProducts');
    Route::get('low_stock', [AnalyticsController::class, 'low_stock'])->name('analytics.lowStock');
    Route::get('summary', [AnalyticsController::class, 'summary'])->name('analytics.summary');
    Route::get('near_expiry', [AnalyticsController::class, 'near_expiry'])->name('analytics.nearExpiry');
});

# Routes for Batches (Logistics)
Route::middleware(['auth:api'])->prefix('delivery/batches')->group(function () {
    Route::get('get_batches', [BatchesController::class, 'index'])->name('batches.getAll');
    Route::get('get_batch/{batch_id}', [BatchesController::class, 'show'])->name('batches.getOne');
});

# Worker-only route: update product quantity only
Route::middleware(['auth:api'])->patch('inventory/product/update_quantity/{product_id}', [ProductController::class, 'update_quantity'])->name('product.updateQuantity');