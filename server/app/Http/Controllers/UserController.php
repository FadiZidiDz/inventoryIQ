...
class UserController extends Controller
{
    ...
    public function logout(Request $request) {
        ...
        // forget specific cache keys that are related to the user
        Cache::forget('navigations_' . $role_id . '_' .$user_id);
        Cache::forget('inventoryControl_subNavigations_' . $role_id . '_' . $user_id);
        Cache::forget('prodDelivery_subNavigations_' . $role_id . '_' . $user_id);
        Cache::forget('profile_subNavigations_' . $role_id . '_' . $user_id);
    ...
    public function create_user(Request $request) {
        ...
        // fallback password when admin leaves it empty
        $password_length = 8;
        $characters = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!@#$%^&*()-_';
        $shuffledChars = str_shuffle($characters);
        $randomString = substr($shuffledChars, 0, $password_length);
        $finalPassword = $request->filled('password') ? $request->password : $randomString;
        ...
        try {
            ...
            // Try to send email, but do NOT rollback user creation if mail fails
            try {
                Mail::to($user->email)->send(new UserRegistrationMail($user_data_mail));
            } catch (\Exception $mailException) {
                // Log mail error and continue
                \Log::warning('UserRegistrationMail failed: ' . $mailException->getMessage());
            }
            ...
            return response()->json([
                'message' => $request->filled('password')
                    ...
                    : 'User registered successfully. Share the generated password with the user.',
                'generated_password' => $request->filled('password') ? null : $randomString
            ]);
        } catch (\Exception $e) {
            DB::rollback();
