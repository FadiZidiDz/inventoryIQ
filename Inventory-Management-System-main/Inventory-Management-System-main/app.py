...
def ml_alerts():
    """Endpoint for stock alerts (low stock, near expiry)"""
    try:
        if not os.path.exists(app.config['DATA_PATH']):
            return jsonify({"alerts": [], "message": "No data file"}), 200
        
        df = pd.read_csv(app.config['DATA_PATH'])
        from utils import get_stock_alerts
        alerts = get_stock_alerts(df)
...
def predict_batch():
    """Predict stock for multiple items at once"""
    try:
        data = request.get_json()
